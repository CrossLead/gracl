...
export class Resource extends Node {
  ...
  async getPermission(subject: Subject): Promise<Permission> {
    ...
    return (
      permissions[permissionIndexOf(permissions, subjectId)] ||
      (<Permission>{
        subjectId,
        resourceId: '',
        resourceType: '',
  ...
  async determineAccess(
    ...
    permissionsToCheck.forEach(perm => {
      accessResults[perm] = {
        ...
        access: false,
        reason:
          'No permissions were set specifically for this subject/resource combination.'
      };
    });
    ...
    while (true) {
      ...
      while (true) {
        for (let i_p = 0, l_p = permissionsToCheck.length; i_p < l_p; i_p++) {
          ...
          for (let i_s = 0, l_s = currentSubjects.length; i_s < l_s; i_s++) {
            ...
            for (let i_r = 0, l_r = currentResources.length; i_r < l_r; i_r++) {
              ...
              if (
                (access === true && !finishedPermissions.has(perm)) ||
                access === false
              ) {
        ...
        if (
          ...
          currentSubjects.some(s => s.hierarchyRoot())
        )
          break;
  ...
  async updatePermission(
    ...
    if (existingPermissionIndex >= 0) {
      permissions[existingPermissionIndex] = action(
        permissions[existingPermissionIndex]
      );
    } else {
      // add permission
      permissions.push(
        action({ subjectId, resourceId, resourceType, subjectType })
      );
    }

    // save updated document
    const id = this.getId(),
      updated = await CurrentResourceClass.repository.saveEntity(
        id,
        this.doc,
        this
      );
